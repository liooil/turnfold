  ...
  if (request.mode === "navigate") {
    event.respondWith(fetch(request).then((response) => {
      if (response.ok) {
        ...
        void caches.open(cacheName).then((cache) => cache.put("/turnfold/app/", cacheable)).catch(() => {});
      }
      return response;
    }).catch(async () => (await caches.match("/turnfold/app/")) || Response.error()));
    return;
  }
