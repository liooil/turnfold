...
export function createThreadView(state: AppState, dependencies: ThreadDependencies) {
  ...
  function renderMessageMarkup(message: StoredChatMessage, index: number, replyTarget: boolean, streamed: boolean) {
    ...
    if (message.role === "user") {
      return `<article class="message user-message${replyTarget ? " reply-target" : ""}" ${messageAttributes}><div class="message-content user-content"><p>${escapeHtml(messagePartText(message, "text"))}</p></div><div class="user-message-actions">${replyAction(message, index)}<button class="icon-button" type="button" data-action="regenerate-message" data-index="${index}" aria-label="让 AI 重新回复这条消息" title="让 AI 重新回复这条消息"${state.streaming ? " disabled" : ""}>${dependencies.icons.regenerate}</button><button class="icon-button" type="button" data-action="edit-message" data-index="${index}" aria-label="编辑消息" title="编辑消息"${state.streaming ? " disabled" : ""}>${dependencies.icons.edit}</button>${branches}</div></article>`;
    }
  ...
  function renderBranchNavigator(message: StoredChatMessage) {
    ...
    return `<span class="branch-navigator" aria-label="分支 ${index + 1}/${siblings.length}"><button class="icon-button" type="button" data-action="switch-branch" data-id="${escapeHtml(previous.id)}" aria-label="上一个分支" title="上一个分支">‹</button><span>${index + 1} / ${siblings.length}</span><button class="icon-button" type="button" data-action="switch-branch" data-id="${escapeHtml(next.id)}" aria-label="下一个分支" title="下一个分支">›</button></span>`;
  }
  ...
  function renderBranchPreviewNotice() {
    ...
    return `<div class="branch-preview-notice"><span>正在查看非当前分支</span><div><button type="button" data-action="leave-branch-preview" title="返回当前分支">返回当前分支</button><button type="button" data-action="confirm-branch-preview" title="将当前会话切换到这里">将当前会话切换到这里</button></div></div>`;
  }
  ...
  function renderThread() {
    ...
    return `<section class="thread-root"><div class="thread-viewport" id="thread-viewport">${renderBranchPreviewNotice()}<div id="message-list"></div><div class="thread-footer${fullscreen ? " fullscreen-editor" : ""}">${fullscreenHeader}<button class="scroll-button" type="button" data-action="scroll-bottom" aria-label="滚动到底部" title="滚动到底部">${dependencies.icons.scroll}</button>${dependencies.renderWorkingPanel()}${editing ? `<div class="edit-context"><span>正在编辑${editRole}；提交后当前会话将从这里继续</span><button type="button" data-action="cancel-edit" title="取消编辑">取消</button></div>` : ""}<form class="composer" id="composer"><textarea class="composer-input" name="message" placeholder="${placeholder}" rows="1" aria-label="聊天消息">${escapeHtml(draft ? workingItemText(draft) : "")}</textarea><div class="composer-actions">${state.streaming ? `<button class="stop-button" type="button" data-action="stop" aria-label="停止生成" title="停止生成">${dependencies.icons.stop}</button>` : ""}<button class="fullscreen-button" type="button" data-action="toggle-composer-fullscreen" aria-label="${fullscreen ? "退出全屏编辑" : "进入全屏编辑"}" title="${fullscreen ? "退出全屏编辑" : "全屏编辑"}"${fullscreen ? "" : " hidden"}>${fullscreen ? dependencies.icons.collapse : dependencies.icons.expand}</button></div>${dependencies.renderModelPicker()}<button class="send-button" type="submit" data-action="send" aria-label="${state.streaming ? "排队发送" : "发送消息"}" title="${state.streaming ? "排队发送" : "发送消息"}">${dependencies.icons.send}</button></form><p class="composer-note${state.offline ? " offline" : ""}${queued ? " queued" : ""}">${note}</p></div></div></section>`;
  }
